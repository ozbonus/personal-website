...
<PostPicture src={rules1} alt="Plus, minus, times, and equals cards are laid out on the table." title="Operators" />
...
<PostPicture src={rules2} alt="A 4 tens card and a 2 ones card are concatenated to make 42." title="Target" />
...
<PostPicture src={rules4} alt="Three plays cards, 5, 8, and 9 are on the table. The players are thinking." title="Game start" />
...
<PostPicture src={rules5} alt="The dealer asks, 'Would you like another card?'.  One player says, 'Yes' and another says, 'OK'." title="Getting stuck" />
...
<PostPicture src={rules6} alt="The play cards 5, 8, 9, and now 3 are on the table." title="Another card" />

⤷ The dealer, with some dramatic flare, places another play card on the table.

<PostPicture src={rules7} alt="A player raises their hand and shouts, 'I know!'" title="Eureka!" />

⤷ One player thinks that they have a solution and signals to the dealer.

<PostPicture src={rules8} alt="The player says, 'Nine times five is forty-five. Forty-five minus three is forty-two.'" title="Calculating" />
...
<PostPicture src={rules9} alt="The play cards 5, 9, and 3 have been removed from the table. The play card 8 remains." title="Three points" />
...
<PostPicture src={personal2} alt="Left: My personal deck of cards. The differently-cut corners make the different groups of cards easy to distinguish. Right: A 10 millimeter corner cutter." title="Cutting corners" />
...
<PostPicture src={distribution} alt="A roughly normal distribution, truncated at 0 and 99. It's stepwise, with each step comprising 10 numbers. The highest step is the one that covers 20 to 29." title="Target distribution" />
